...
function handleLoginStatus(
  ...
  // 已登录或访问登录页或标记为无需登录的路由，直接放行
  if (userStore.isLogin || to.path === RoutesAlias.Login || to.meta.ignoreAuth) {
    return true
  }
...
async function handleDynamicRoutes(to: RouteLocationNormalized, next: NavigationGuardNext, router: Router): Promise<void> {
  ...
  try {
    ...
    let { path: validatedPath, hasPermission } = RoutePermissionValidator.validatePath(to.path, menuList, homePath.value || '/')
