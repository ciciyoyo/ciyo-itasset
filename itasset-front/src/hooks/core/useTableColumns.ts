...
/**
 * 获取特殊列标签
 * 使用函数而不是常量，以确保在i18n初始化后才访问翻译
 */
const getSpecialColumnLabel = (type: string): string => {
    const {t} = useI18n()
 ...
 * 特殊列类型配置
 */
const SPECIAL_COLUMNS: Record<string, { prop: string; getLabel: () => string }> = {
    selection: {prop: '__selection__', getLabel: () => getSpecialColumnLabel('selection')},
    expand: {prop: '__expand__', getLabel: () => getSpecialColumnLabel('expand')},
    index: {prop: '__index__', getLabel: () => getSpecialColumnLabel('index')}
}
...
export const getColumnChecks = <T>(columns: ColumnOption<T>[]) =>
    columns.map((col) => {
        ...
        if (special) {
            return {...col, prop: special.prop, label: special.getLabel(), checked: true, visible: true}
        }
