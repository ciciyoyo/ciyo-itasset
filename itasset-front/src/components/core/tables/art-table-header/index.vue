...
<template>
  <div class="flex-cb max-md:!block" id="art-table-header">
    ...
    <div class="flex-c md:justify-end max-md:mt-3 max-sm:!hidden">
      ...
      <div v-if="shouldShow('refresh')" class="button" @click="refresh" :class="{ loading: loading && isManualRefresh }">
        <ArtSvgIcon icon="ri:refresh-line" :class="loading && isManualRefresh ? 'animate-spin text-g-600' : ''" />
      </div>
      ...
      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        ...
        <template #dropdown>
          <ElDropdownMenu>
            ...
            >
              <ElDropdownItem :key="item.value" :command="item.value" :class="tableSize === item.value ? '!bg-g-300/55' : ''">
                {{ item.label }}
              </ElDropdownItem>
            </div>
          </ElDropdownMenu>
