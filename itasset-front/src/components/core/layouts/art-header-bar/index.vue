...
<template>
  <div
    ...
    :class="[tabStyle === 'tab-card' || tabStyle === 'tab-google' ? 'mb-5 max-sm:mb-3 !bg-box' : '']"
  >
    <div
      class="relative box-border flex-b h-15 leading-15 select-none"
      :class="[tabStyle === 'tab-card' || tabStyle === 'tab-google' ? 'border-b border-[var(--art-card-border)]' : '']"
    >
      ...
      <div class="flex-c gap-2.5">
        ...
        <!-- 通知按钮 -->
        <div ref="userNewsBadgeRef" v-if="shouldShowNotification" class="h-full flex-c">
          <ArtIconButton icon="ri:notification-2-line" class="notice-button relative">
            <div v-if="isDot" class="absolute top-2 right-2 size-1.5 !bg-danger rounded-full"></div>
          </ArtIconButton>
        </div>
        ...
        <!-- 主题切换按钮 -->
        <ArtIconButton v-if="shouldShowThemeToggle" @click="themeAnimation" :icon="isDark ? 'ri:sun-fill' : 'ri:moon-line'" />
    ...
    >
      <ArtNotification v-model:value="isDot" />
    </ElPopover>
  </div>
</template>
