...
<script setup lang="ts">
  ...
  import { deallocateLicense } from '@/api/itam/licenses'
  ...
  const {
    ...
    core: {
      ...
      columnsFactory: () => [
        { type: 'selection' },
        { prop: 'itemName', label: '软件名称', minWidth: 150 },
  ...
  const handleUnbind = (row: AllocationsVO) => {
    ElMessageBox.confirm('确认解除该软件的关联吗？', '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    }).then(async () => {
      try {
        await deallocateLicense(row.id)
