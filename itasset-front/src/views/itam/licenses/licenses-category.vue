      ...
      <ElCard class="art-table-card flex flex-col flex-1 min-h-0" shadow="never">
          ...
          <template #left>
            <ElSpace wrap>
              ...
              <ExcelImport
                ...
                title="导入软件分类"
                :data="{ categoryType: AssetType.LICENSE }"
        ...
        >
          <template #operation="{ row }">
            <el-button link type="primary" @click="handleEdit(row)" v-hasPermi="['itam:licenses:update']">
...
<script setup lang="ts">
  ...
  const form = ref<Partial<CategoriesEntity>>({
    ...
    code: '',
    parentId: undefined,
    categoryType: AssetType.LICENSE
  })
  ...
  const handleDelete = (row: CategoriesEntity) => {
    ElMessageBox.confirm(`确认删除分类"${row.name}"吗？`, t('common.warning'), {
      ...
      .then(async () => {
        await delCategories(row.id, 'licenses')
