    ...
    <div class="log-page art-full-height">
        <div class="flex flex-col h-full">
            <ElCard class="art-table-card flex flex-col flex-1 min-h-0" shadow="never">
                    ...
                    <template #left>
                        <ElSpace wrap>
                            <el-button type="primary" icon="ele-Plus" @click="handleAdd" v-ripple
                                v-hasPermi="['itam:categories:add']">
                                {{ $t('common.add') }}
                            </el-button>
                        </ElSpace>
                    </template>
                ...
                <!-- Tree Table -->
                <ArtTable ref="tableRef" class="flex-1" :loading="loading" :data="categoryTree" :columns="columns"
        ...
        <el-dialog :title="dialogTitle" v-model="open" width="500px" append-to-body>
            <el-form ref="formRef" :model="form" :rules="formRules" label-width="100px">
                <el-form-item label="上级分类" prop="parentId">
                    <el-tree-select v-model="form.parentId" :data="categoryTreeOptions" check-strictly
                        :render-after-expand="false" placeholder="请选择上级分类" clearable node-key="id"
...
const form = ref<Partial<CategoriesEntity>>({
    ...
    code: '',
    parentId: undefined,
    categoryType: AssetType.ACCESSORY
})
