            ...
            <ElCard class="art-table-card flex flex-col flex-1 min-h-0" shadow="never"
                    ...
                    <template #left>
                        <ElSpace wrap>
                            <el-button type="primary" icon="ele-Plus" @click="handleAdd" v-ripple
                                v-hasPermi="['itam:consumables:add']">
                                {{ $t('common.add') }}
                            </el-button>
                            ...
                            <el-button v-hasPermi="['itam:consumables:export']" icon="ele-Download" v-ripple
                                @click="handleExport">
                                {{ $t('common.export') }}
                            </el-button>
                        </ElSpace>
                ...
                <ArtTable class="flex-1" :loading="loading" :data="data" :columns="columns" :pagination="pagination"
                    ...
                    <template #operation="{ row }">
                        <el-button link type="primary" @click="handleCollect(row)"
                            v-hasPermi="['itam:consumables:collect']">
                            领取
                        </el-button>
                        <el-button link type="success" @click="handleStockIn(row)"
                            v-hasPermi="['itam:consumables:stockIn']">
                            入库
                        </el-button>
                        <el-button link type="primary" @click="handleUpdate(row)"
                            v-hasPermi="['itam:consumables:update']">
        ...
        <el-dialog :title="dialogTitle" v-model="open" width="500px" append-to-body>
            <el-form ref="consumablesRef" :model="form" :rules="formRules" label-width="120px">
                ...
                <el-form-item label="当前库存数量" prop="quantity">
                    <el-input-number v-model="form.quantity" placeholder="请输入当前库存数量" controls-position="right" :min="0"
                        class="!w-full" />
                </el-form-item>
                <el-form-item label="最小库存预警" prop="minQuantity">
                    <el-input-number v-model="form.minQuantity" placeholder="请输入最小库存预警" controls-position="right"
                        :min="0" class="!w-full" />
                </el-form-item>
        ...
        <el-dialog title="耗材领取" v-model="collectDialogVisible" width="400px" append-to-body>
            <el-form ref="collectFormRef" :model="collectForm" :rules="collectRules" label-width="100px">
                ...
                <el-form-item label="领取数量" prop="quantity">
                    <el-input-number v-model="collectForm.quantity" :min="1" :max="Number(currentItem?.quantity) || 0"
                        controls-position="right" class="!w-full" />
                </el-form-item>
                <el-form-item label="领取人" prop="targetId">
                    <el-select v-model="collectForm.targetId" placeholder="请选择领取人" @click="handleOpenCollectUserTable"
                        class="!w-full">
                        <el-option v-if="collectUser" :key="collectUser.id" :disabled="true"
                            :label="collectUser.nickName" :value="collectUser.id" />
                    </el-select>
                </el-form-item>
        ...
        <el-dialog title="耗材入库" v-model="stockInDialogVisible" width="400px" append-to-body>
            <el-form ref="stockInFormRef" :model="stockInForm" :rules="stockInRules" label-width="100px">
                ...
                <el-form-item label="入库数量" prop="quantity">
                    <el-input-number v-model="stockInForm.quantity" :min="1" controls-position="right"
                        class="!w-full" />
                </el-form-item>
                <el-form-item label="备注" prop="note">
