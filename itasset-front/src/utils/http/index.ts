...
axiosInstance.interceptors.request.use(
  ...
  (error) => {
    showError(createHttpError(i18n.global.t('httpMsg.requestConfigError'), ApiStatus.error))
...
function handleUnauthorizedError(message?: string): never {
  const error = createHttpError(message || i18n.global.t('httpMsg.unauthorized'), ApiStatus.unauthorized)
